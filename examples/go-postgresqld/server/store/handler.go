...
func (store *MemStore) ParserError(conn *postgresql.Conn, q string, err error) (message.Responses, error) {
	IsMatchQuery := func(q string, queryPrefix string) bool {
		return q[:len(queryPrefix)] == queryPrefix
	}
	switch {
	case IsMatchQuery(q, pgbenchGetPartitionQuery):
		return newPgbenchGetPartitionResponse()
	}
...
func newPgbenchGetPartitionResponse() (message.Responses, error) {
